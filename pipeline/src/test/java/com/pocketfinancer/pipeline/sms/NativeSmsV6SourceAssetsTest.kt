...
class NativeSmsV6SourceAssetsTest {
    @Test fun automaticSourceBundleMatchesEveryFrozenManifestHash() {
        val root = listOf(
            File("src/main/assets/sms_processing/native-integration-v6"),
            File("pipeline/src/main/assets/sms_processing/native-integration-v6")
        ...
        assertEquals(NativeSmsV6Assets.MANIFEST_SHA256, sha256(bytes))
        val manifest = JSONObject(bytes.toString(Charsets.UTF_8))
        assertEquals(NativeSmsV6Assets.RELEASE_ID, manifest.getString("release_id"))
        assertTrue(manifest.getBoolean("automatic_persistence_enabled"))
        val artifacts = manifest.getJSONArray("artifacts")
        assertEquals(54, artifacts.length())
