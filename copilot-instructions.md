...
2. **Background processing (Implemented)**:
   - `SmsReceiver` delegates incoming alerts to a unique WorkManager chain. Automatic SMS processing defaults off and is owned by `AutomaticProcessingPreferences`; users must explicitly opt in. After a durable claim, Home publishes an immediate, process-only, inspect-only activity card keyed by candidate and claim token; it can coexist with manual or historical activity and is cleared exactly when the claim ends. Disabling automatic intake discards pending work while allowing an already claimed SMS to finish. Manual scans remain available and can discover messages received while automatic processing was off.
